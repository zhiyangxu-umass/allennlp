...
class SelfTrainingTrainer(Trainer):
    ...
    def __init__(
        ...
        aux_data_loader: DataLoader = None,
        warm_up_epoch: int = -1,
        mix_ratio: float = 1.0,
        no_gold: bool = False,
        ...
        **kwargs,
        ...
        self.aux_data_loader = aux_data_loader
        self._warm_up_epoch = warm_up_epoch
        if self.aux_data_loader is not None:
            self.aux_data_loader.set_target_device(self.cuda_device)
            self._mix_ratio = mix_ratio
            self._weighted_self_training = weighted_self_training
            self._no_gold = no_gold
    ...
    def batch_outputs(self, batch: TensorDict, for_training: bool, aux_batch: TensorDict = None) -> Dict[str, torch.Tensor]:
        """
    ...
    def _train_epoch(self, epoch: int, warm_up: bool=False) -> Dict[str, float]:
        ...
        if True:
            ...
            batch_aux_generator = iter(self.aux_data_loader)
            batch_aux_group_generator = common_util.lazy_groups_of(
                batch_aux_generator, self._num_gradient_accumulation_steps
            )
        # if aux data is provided
            for batch_group, aux_group in zip(batch_group_generator_tqdm, batch_aux_group_generator):
                if done_early:
                    break
    ...
    def _try_train(self) -> Tuple[Dict[str, Any], int]:
        ...
        for epoch in range(self._num_epochs):
            ...
            warm_up = epoch < self._warm_up_epoch
            # allows to evaluate the last warmup epoch
            train_metrics = self._train_epoch(epoch, warm_up= epoch<=self._warm_up_epoch)
            ...
            if self._validation_data_loader is not None and not warm_up:
            # if self._validation_data_loader is not None and not warm_up and epoch%5 == 0:
                with torch.no_grad():
    ...
    def from_partial_objects(
        ...
        **kwargs,
        ...
        return cls(
            ...
            mix_ratio=mix_ratio,
            no_gold=no_gold,
            weighted_self_training=weighted_self_training,
            patience=patience,
